...
FIRERED_DIR = "/root/FireRed-Image-Edit"
os.environ["GRADIO_CDN_BACKEND_ENABLED"] = "False"
os.environ["GRADIO_UPLOAD_CHUNK_SIZE"] = "8388608"  # 8 MB chunks — fixes 504 timeout on gradio.live tunnel
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"  # reduces fragmentation for 17GB transformer + 5GB activations
...
def _run_unirig(glb_path: str, out_dir: str) -> str:
    ...
    def _launch(script: str, extra_args: list[str]):
        ...
        r = subprocess.run(cmd, cwd=UNIRIG_DIR, capture_output=True, text=True,
                           timeout=300, env=env)
        if r.returncode != 0:
    ...
    _launch("merge.sh", ["--source", skin_fbx, "--target", glb_path, "--output", rigged])
...
def gradio_rig(
    ...
    try:
        ...
        if input_image is not None:
            try:
                _meshforge_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "MeshForge")
                ...
                progress(0.6, desc="Stage 2/3: PSHuman — RMBG + multi-view face generation...")
                ...
                generate_pshuman_mesh(image_path=img_path, output_path=face_obj, service_url="direct")

                progress(0.85, desc="Stage 3/3: Face transplant — stitching into rigged mesh...")
                ...
                print(f"[rig] PSHuman stage failed, using plain rig: {_pse}\n{traceback.format_exc()}")
                final = rigged
        ...
        status_msg = ("Rigged + PSHuman HD face: " if had_pshuman else "Rigged: ") + os.path.basename(final)
        if fbx:
...
def _portrait_to_rgba(img_pil: Image.Image) -> Image.Image:
    ...
    img_t = _TF.normalize(img_t, [0.485, 0.456, 0.406],
                                  [0.229, 0.224, 0.225]).unsqueeze(0)
    with torch.no_grad():
...
with gr.Blocks(title="TripoSG + MV-Adapter 3D Studio", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs() as tabs:
        ...
        with gr.Tab("Rig & Export"):
            ...
            rig_btn.click(
                fn=gradio_rig,
                inputs=[input_image, glb_state, export_fbx_check,
                        pshuman_weight_thresh, pshuman_retract_mm],
                outputs=[rig_glb_dl, gr.State(None), rig_fbx_dl, rig_status,
                         rig_model_3d, rigged_glb_state, gr.State(None)],
            )
