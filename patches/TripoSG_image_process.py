...
def load_image(img_path, bg_color=None, rmbg_net=None, padding_ratio=0.1):
    ...
    def rmbg(image: torch.Tensor) -> torch.Tensor:
        # Try RMBG-2.0 (BiRefNet) with ImageNet norm
        img_v2 = TF.normalize(image, [0.485,0.456,0.406], [0.229,0.224,0.225]).unsqueeze(0)
        result = rmbg_net(img_v2)
        # RMBG-2.0: flat list of tensors, each [B,1,H,W]; last = finest
        # RMBG-1.4: list of lists, result[0] is a list not a tensor
        if isinstance(result, (list, tuple)) and len(result) > 0 and isinstance(result[0], torch.Tensor):
            return result[-1][0]   # [B,1,H,W] -> [1,H,W]
        # Fallback: RMBG-1.4 with its own normalization
