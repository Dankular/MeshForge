...
class MDM(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if 'target_cond' in y.keys():
            time_emb += self.mask_cond(self.embed_target_cond(y['target_cond'], y['target_joint_names'], y['is_heading'])[None], force_mask=y.get('target_uncond', False))
    ...
    # Patched: propagate _apply / train to DummySMPL so .to(device) / .train()
    # don't crash when SMPL weights are unavailable.
    def _apply(self, fn):
        super()._apply(fn)
        sm = self.rot2xyz.smpl_model
        if sm is not None and hasattr(sm, '_apply'):
            sm._apply(fn)
        return self
    ...
    def train(self, *args, **kwargs):
        ...
        sm = self.rot2xyz.smpl_model
        if sm is not None and hasattr(sm, 'train'):
            sm.train(*args, **kwargs)
        return self
...
class EmbedTargetLocSingle(nn.Module):
    def __init__(self, all_goal_joint_names, latent_dim, num_layers=1):
        ...
        self.target_cond_dim = len(self.extended_goal_joint_names) * 4
        self.latent_dim = latent_dim
